    ...
    return (
        <div className="container mx-auto px-4 py-6">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-12">
                {/* Left: Gallery */}
                <ProductGallery images={product.images} model3D={product.model3D} />
                        ...
                        <div className="flex items-center gap-4 mt-4">
                            <div className="flex items-center gap-1">
                                {[...Array(5)].map((_, i) => (
                                    <Star
                                        key={i}
                                        className={`h-4 w-4 ${i < Math.floor(product.rating) ? 'fill-yellow-400 text-yellow-400' : 'text-border'}`}
                                    />
                                ))}
                                ...
                                {product.colors.map((color) => (
                                    <button
                                        ...
                                        className={`w-10 h-10 rounded-full border-2 hover:scale-110 transition-all focus:outline-none relative
                                            ${selectedColor === color.name ? 'border-foreground ring-2 ring-foreground/20' : 'border-border'}`}
